...
const EmployeesPage = ({
  ...
  const openEdit = (emp) => { setForm(removePasswordFields(emp)); setModal("edit"); };
  ...
  const save = async () => {
    ...
    try {
      ...
      const sanitizedForm = removePasswordFields(form);
      ...
      if (modal === "add") {
        ...
        await updateEmployee(form.id, { ...sanitizedForm, ...locked, basic: +form.basic, allowances: +form.allowances });
      }
      setModal(null);
    } finally {
      setSaving(false);
                  ...
                  <td style={{ padding: "12px 16px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      {(user.role === "admin" || user.role === "hr" || canManageEmployee(user, emp)) && (
                        <button onClick={() => openEdit(emp)} style={{ background: T.warningTint, border: "none", color: T.warning, padding: "5px 8px", borderRadius: 6, cursor: "pointer" }}><Edit   size={13} /></button>
                      )}
